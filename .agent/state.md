...
_Last updated: 2026-09-15, after **ORDER-FIX-0915 — the follow-ups: items
73, 74, 75, 76 fixed one per commit on `fix/followups-0915` (pushed as the
record, NOT merged), and the deployment-id trap killed — docs-only commits no
longer deploy, so the serving deployment is the latest CODE commit.** Before
...
70 annotated. **Merge:** **`main` = `live` = `f3d5a83`** (fast-forward, 2026-09-15 09:43Z: the gated code tip `d793cb1` plus the TEST-0915 close-out commit), production `2.malaky.ai` and the `live` preview `1.malaky.ai` on one bundle `index-BPJU2leT.js`, the post-deploy smoke green (`Docs/qa/test-0915/run-log.md` Phase 5, `post-deploy/`). **From ORDER-FIX-0915 on, docs-only commits (`.agent/`, `Docs/`, the root markdown files) do not deploy** (`vercel.json` `ignoreCommand`, D-FIX-0915-D), so the serving deployment of a branch is its latest CODE commit, not its tip; the pair is recorded as commit SHAs here, never as deployment ids.
