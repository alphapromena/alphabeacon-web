...
_Last updated: 2026-09-15, after **TEST-0915-2 — the second named testing
session: the ignored build step fixed (it diffs against the last successful
deployment, D-TEST-0915-2-A), two gates clean except buckets c and d, the
persistent shell's first live exercise, and the follow-ups stack
(`fix/followups-0915` + `feat/shell-0915`) MERGED to `main` and `live` —
main = live = `b1783d6` (NIGHT-0916: six orders + the session), one bundle `index-Dtgnuihw.js` on both hosts, the post-deploy smoke
green.** Before that, **ORDER-FIX-0915 — the follow-ups: items
73, 74, 75, 76 fixed one per commit on `fix/followups-0915`, and the
deployment-id trap killed — docs-only commits no longer deploy, so the serving
deployment is the latest CODE commit.** Before
that, **TEST-0915 — the named testing session on
...
70 annotated. **Merge:** **`main` = `live` = `b1783d6`** (fast-forward, 2026-09-16, NIGHT-0916: orders 1–6 — items 81 client side, 83, returnTo, 82, 78, the login panel — and TEST-0916, on top of `3b5d72c`), production `2.malaky.ai` and the `live` preview `1.malaky.ai` on one bundle `index-Dtgnuihw.js`; the close-out docs commit CANCELED on both (D-FIX-0915-D). Before that **`3b5d72c`** (fast-forward, 2026-09-15, TEST-0915-2: the follow-ups stack — items 73–77, the ignored build step — on top of `f3d5a83`), production `2.malaky.ai` and the `live` preview `1.malaky.ai` on one bundle `index-BVhSvkNX.js`, both deployments READY because their base was the last successful deployment and the diff carried code; the docs commit that followed was CANCELED on both, the hosts unchanged — the deployment-id trap closed on production, on the record. Post-deploy smoke: green on every row in run 4 (theme, fresh signup with 000000, first light once and not on reload, five screens in one frame with zero console errors, the tagged indicator travelling, the toast at 13.62:1 from its first frame, a mid-session 401 on login) (`Docs/qa/test-0915-2/run-log.md` Phase 6, `post-deploy/` on disk). Before that **`f3d5a83`** (TEST-0915, 09:43Z, `index-BPJU2leT.js`). **From ORDER-FIX-0915 on, docs-only commits (`.agent/`, `Docs/`, the root markdown files) do not deploy** (`vercel.json` `ignoreCommand`, D-FIX-0915-D), so the serving deployment of a branch is its latest CODE commit, not its tip; the pair is recorded as commit SHAs here, never as deployment ids.
...
`1772734`, pushed as the record and MERGED 2026-09-10 (the end of this paragraph). Hasan's document verbatim at
...
`*.vercel.app` is unreachable from this host (item 53). **Phase 0 ACCEPTED (the founder, 2026-09-10): A1 confirmed, A2 amended to Higgsfield card art (fetched, 13 WebPs under `public/studio/cards/`), A3 PROVEN by the funded proofs on org 1813 (`--funded-proofs`, 14 cents: logos, voice with its audio + document envelope, images.edit fetching OUR url); items 57/58 filed for Hasan; the branch pushed to origin as the record.** **/A, /C and /D are BUILT on this branch (`37e8fa8`, pushed as the record): the one table `src/data/media-capabilities.ts`, the grid and the capability composers, the org id in Settings, `assertNotProduction` in the harness with `E2E_API_ENV=dev` required and `Docs/api/environments.md`; the W5 model composer kept for D4 and E4 only. THE GATE IS CLOSED (2026-09-10, tip `8fed147` + the journal): lint · typecheck · guard-static 352 · unit **633 / 55** · static e2e **115 / 84 skipped / 0** (six verify runs and a solo run; the chain's bare-run red was the harness's) · `verify:w00`–`w06` **all PASS** · live round 1 **18/21** (two spec defects of this series, fixed and re-run green; one API red on brand-rules) · **live round 2 — the gate — 20/21 green, the one red a storage-PUT `ECONNRESET` (network-lost, re-run 3/3 as a recorded supplement)**; the host held awake both rounds; `E2E_API_ENV=dev` on every file. Record: `Docs/qa/hsn-0910/gate/`. **MERGED and DEPLOYED 2026-09-10 on the founder's word: `main` and `live` fast-forwarded `1772734` / `463806c` → `83fc942` (the classifier allowed `main:live` today); production `dpl_4HmK6thTkPezvYMHSQuuhwputwtc` and the `live` preview `dpl_BvXcdyDLNdeRVikHriArxYaWFmMD` READY at `83fc942`; the bundle `index-Ckpi_DKM.js` on BOTH `1.malaky.ai` and `alphabeacon-web.vercel.app` (the production bundle measured for the first time, from Vercel's side through the MCP fetch) — so production `main` is LIVE on the dev API base: `VITE_API_BASE_URL` was set in the Production scope by hand at 10:51:50Z, two minutes before the push (item 53's STATIC record corrected); `www.malaky.ai` was added to the project at 10:43:24Z (verified, 308 → the apex); the apex waits on the founder's GoDaddy hand — `A malaky.ai 216.198.79.1` + `A malaky.ai 64.29.17.1` replacing the site builder's two A records, `CNAME www.malaky.ai c61f41105463f8af.vercel-dns-017.com.`, no TXT; when it lands the apex serves the dev API in public (item 53). Rollback candidate: the `1772734` pair, `dpl_231WHducztuK5ZpYho8abm21Ygjf` (production) and `dpl_7Do9vv6tEkPN25W313k1arfmsViT` (`live`, `463806c`). **M-HSN-2 PASSED on production (the founder, 2026-09-10): the grid, the Logos and Image edit composers, one balanced logos render (wallet −3 cents, the catalog's $0.03), the org id in Settings — item 56 CLOSED.** Next: items 51, 57, 58 to Hasan and 52, 53 to Ward (the founder sends); the DNS cutover by the founder's hand; /B when Hasan names the door.** **GATE-0910 Phase 0 DONE 2026-09-10 (branch `feat/gate-0910` off `4d98942`, local only, report-and-stop): the 21 live specs laned (A 15 files / 717 s, B 6 / 387 s; a round projected ~10–11 min against 17.2 measured; end to end ~25 min against ~60), the seven verifies read (the same six suite runs ×7 = 19 min; 33 own assertions and the manual lists kept), the `vite preview` probe NOT identical — live-auth test 3 red twice on the production build and green on the dev server: a dev-mode artefact (the dev world's "Atlas Roasters" becomes the workspace name; item 59), the trap-22 tripwire blind on a preview server; nine dormant live tests (item 60). Record `Docs/qa/gate-0910/phase0/`. **§3 BUILT on the founder's word (2026-09-10; item 59 both fixes, billing in B / wallet in A, item 60's skips counted and `--funded`): verify-once — `pnpm verify:all` + the seven checks over its report, the static half 21 min → 170 s measured; `pnpm gate` (`scripts/gate/`) — the two lanes (16 A / 5 B, `lanes.ts` proven against the disk), one production build served by one runner-owned `vite preview` per round, the tripwire preview-aware, network-lost re-runs 3/3, every skip with its reason, the record worked under `.gate/runs/` and published under `Docs/qa/<series>/gate/<run>/`; the org pool behind `--pool`, OFF; `runStamp()` salts every live file's emails; the app never takes a workspace name from the demo world and live-auth expects N3. THE WORKER COUNT MEASURED: 3 and 2 in flight red on the dev function's concurrency cap (`429 ConcurrentInvocationLimitExceeded`, item 61 for Ward; the runner warms once and keeps one probe), so lane A runs at 1 until Ward raises it. **§4 PROVEN and CLOSED at two runs (the founder, 2026-09-13 — no fourth re-proof, ever): the old chain 61.8 min against `pnpm gate` 34.8 and 32.9 min on one tree, same unit and static counts, the runner red on four files the old chain cannot see because it serves the PRODUCTION build. Three of those were spec defects (the dev demo world pre-filled a schedule model and an offer line production leaves empty) — fixed, 11 passed on the built app; the fourth is the wire (item 63: the `durationS` TYPE is refused before the wallet, the MAXIMUM after it — measured at zero spend with request-ids, no spec edit). Two reporting fixes: a not-run is no longer counted as a skip, and the app's own error page joins a lost socket as re-runnable. Unit 666/58. Items 62 and 63 filed. **MERGED and DEPLOYED 2026-09-13: `main` fast-forwarded `4d98942` → `387391d` and pushed with `main:live`; the old chain RETIRED (no verify re-runs a suite, no flag makes it, the legacy runners kept only as the §4 record); production `dpl_Dv7y6DJ5Mwc4TfsTJ21pb9Pr2FnQ` and the `live` preview `dpl_5zwntMoMKTx8WVE9zsA9DSKceQeQ` READY at `387391d`, the bundle `index-Ckpi_DKM.js` → `index-CeQdUZ1H.js` on BOTH hosts; `feat/gate-0910` kept on origin as the record; rollback candidate the `4d98942` pair. Neither reporting fix has shaped a real record yet — the first testing session proves them.** **STANDING RULE from the next order: a build order ends at lint, typecheck, unit and static — live rounds and gate runs only in a testing session the founder names.** HSN-0910/B waits for GATE-0910's merge by its base clause.** No new trap;
...
| `feat/hsn-0910` | **HSN-0910, 2026-09-10 — branched off `main` (`1772734`), pushed as the record, MERGED as a fast-forward into `main` and `live` 2026-09-10 (`83fc942`) on the founder's word and kept on origin as the record; Phase 0 DONE at zero spend on QA orgs 1823/1824 and HELD at stop point 1 awaiting the founder: Hasan's document verbatim (`Docs/api/media-capabilities.md`), `scripts/probe-hsn-0910.ts` (`pnpm probe:hsn-0910`), the record under `Docs/qa/hsn-0910/phase0/` and the shapes section. /A, /C, /D not started; /B held on item 51.** |
| `feat/gate-0910` | **GATE-0910, 2026-09-10 — branched off `main` (`4d98942`), LOCAL ONLY (never pushed), Phase 0 DONE (measure and classify) and HELD at the order's §2 stop: the record under `Docs/qa/gate-0910/phase0/`; items 59, 60. The build (§3: verify-once, the two lanes, one server per round, the org-pool flag off, `pnpm gate`) and the proof (§4) wait for the founder's word.** |
