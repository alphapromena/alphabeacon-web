...
| `QA_FUNDED_EMAIL` · `QA_FUNDED_PASSWORD` | shell, dev machine only (the QA-creds store; never committed) | The designated FUNDED QA org's owner sign-in (BIL-0902/R §4 — minted at M-BIL-1 step 8 by a real test-mode checkout, card 4242). When both are set, the generating TEXT specs run there instead of self-skipping on a zero wallet (`skipUnlessFunded` in `e2e/live-setup.ts`). Media renders stay behind `LIVE_MEDIA`. Never read by app code. |
