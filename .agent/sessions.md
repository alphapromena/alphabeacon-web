...
### 2026-09-13 10:15 — ORDER VOICE-0913: the brand voice read/write asymmetry, the cap, and the honest save

- Did: **Three fixes on one root cause**, branch `feat/voice-0913` off `main` = `dff402d`, BUILD ORDER — pushed, NOT merged.
  **The cause, measured on org 1867 the same day:** two voice rows both named `Brand voice` (293 with 18 rules, 294 with 94). `adaptBrand` FLATTENED every row's rules while `saveBrandVoice` PATCHed only the canonical one, so the screen showed the merge and every save wrote that merge back onto one row — **18 rules became 94 across five saves**. Past the wire's documented 50-rule limit (`Docs/api/api.md` line 675, "at most **50** rules per create/PATCH" — the client never enforced a limit its own contract carried) every PATCH answered `400 validation_failed` "Too big: expected array to have <=50 items" and the screen still said **"Brand voice saved"**.
  **(1) Read and write now address the same thing.** `adaptBrand` reads the CANONICAL row's rules and only those; the canonical row is resolved in CREATION order so an org with two same-named rows keeps editing the OLDEST and the write target never moves. Extra rows are NOT hidden — the server builds its context bundle from every voice row, so rules the screen stopped showing would go on shaping drafts invisibly. `BrandGraft.extraVoiceRows` (id, name, rule count; empty rows excluded) carries through the provider and the screen says out loud that they exist, still apply, and are out of its reach. **No migration, deliberately** — merging or deleting rows is destructive and a judgement call; item 66 carries the two candidate mechanisms for the founder.
  **(2) A product cap of 40, COMBINED across Do and Don't**, below the wire's 50 — `MAX_BRAND_VOICE_RULES` beside `MAX_FOLLOWED_SOURCES`/`MAX_TOPICS`, `MESSAGES.errors.brandVoiceCapReached` beside the other two. HSN-04's shape exactly: a visible combined counter, both Add controls disabled at the cap, `RuleList` gaining a `disableAdd` prop, a list already above the cap still listed, still removable and still saveable when it SHRINKS (the only way down from 94), nothing ever silently trimmed, and the seam refusing growth past the cap without touching the wire.
  **(3) A failed save can no longer report success.** The screen renders the wire's own FIELD message with its request id — the Studio composer's pattern, `role="alert"` — and `toastSuccess` fires only after a save that actually succeeded; `toastError` now carries the wire's words instead of the catalogue's generic line.
  **The DATA was repaired by hand earlier the same day** at zero spend, before any code changed: `GET` (`7f999660-729c-433e-ac8b-1eabe4b19879`) → dedup to 19 rules (9 do / 10 dont, the Malaky AI branding rule the only one unique to 294) → `PATCH 293` (`42c0b30e-02ce-4404-8079-f3fb37b2677e`) → read-back (`81ec701a-4f8a-4df4-b74f-67bf4acba608`) → `DELETE 294` **204** (`8a9d11b1-1bd6-44a4-affa-baa57eb725cf`) → final read, one row, 19 rules (`bb438a12-3687-4cb0-9457-78a9fc402311`). Five stop points, each aborting before anything irreversible.
- Phase: post-W7 / INT — a defect order, no phase advanced
- Files: `src/data/adapters/brand-adapter.ts`, `src/data/provider.tsx`, `src/data/brand.ts`, `src/data/types.ts`, `src/lib/messages.ts`, `src/features/settings/brand-voice-screen.tsx`, `src/features/settings/field-editors.tsx`, `src/data/adapters/brand-adapter.test.ts`, `src/data/brand-voice.test.ts` (new), `src/features/settings/brand-voice-screen.test.tsx` (new), `.agent/decisions.md`, `.agent/open-items.md`, `.agent/state.md`, `screens4.md`
- Decisions: see decisions.md — **D-INT-B AMENDED: read and write address the SAME voice row** (the 2026-08-17 clause "READS flatten every voice's rules" is marked superseded in place)
- Verify: **lint · typecheck · guard-static 354 · unit 683/683 (60 files, +17) · static e2e — via `pnpm verify:all`, the build order's full end state.** NOTHING LIVE RAN (the standing rule, 2026-09-13): no gate, no live round, no probe against the deployed API beyond the read-only census and the staged 1867 repair that preceded the code.
- Next: items 65 and 66 are the founder's — how a workspace with extra voice rows should be resolved (a reviewed in-product merge, or Hasan making the voice name unique per org, which is the real fix). The branch is pushed and waits; `live-brand-rules.spec.ts` and `live-brand.spec.ts` both walk this screen and neither was touched, so the next testing session the founder names is what proves all three fixes on the deployed API. Item 64 (the `voice-do` prefix also matching `voice-dont-0` in four specs and one helper) is still open and still deliberately unfixed.

### 2026-09-14 10:05 — ORDER VOICE-0913 MERGED and DEPLOYED; the extra-rows notice points at an owner, not support

- Did: **One copy change, then the merge.** The extra-rows notice said "Ask support to merge them into this list" — **there is no support channel in this product** (the founder, 2026-09-14), so it sent the reader nowhere. It now reads "Ask a **workspace owner or admin** to merge them into this list"; the rest of the sentence is byte-identical. The reason is recorded in the comment above the string so the constraint is not rediscovered, and the two docs that called merging "a support action" — decisions.md's D-INT-B amendment and open item 66 — were corrected to "a workspace owner's act" in the same commit, because the copy change made them untrue.
  **Merged `feat/voice-0913` → `main` FAST-FORWARD** (`dff402d` → `aba11be`, two commits, one linear history), pushed to `origin main` and `origin main:live`. The branch is KEPT on origin as the record. **Production `dpl_4JS6QhApPnbjRfzsuuYHce4shrqV` (branch `main`) and the `live` preview `dpl_2bxSdDidn7Yej23LfNzkouZSvqpc` (branch `live`) both READY at `aba11be`, BOTH serving the same bundle `index-DJniABIr.js` (production read from Vercel's side through the MCP fetch because `*.vercel.app` is unreachable from this host; `1.malaky.ai` read directly). The apex `malaky.ai` is STILL the GoDaddy site-builder page (`Server: DPS/2.0.0`) — the DNS cutover is the founder's hand, item 53.** Rollback candidate: the `dff402d` pair.
- Phase: post-W7 / INT — a defect order, no phase advanced
- Files: `src/lib/messages.ts`, `.agent/decisions.md`, `.agent/open-items.md`, `.agent/state.md`, `.agent/sessions.md`
- Decisions: no new decision — decisions.md's D-INT-B amendment (2026-09-13) gains the no-support-channel constraint as the reason the notice points where it does
- Verify: **`pnpm verify:all` RESULT: PASS on the edited tree `2feeddaf4aec` — lint · typecheck · guard-static 354 · unit 683/683 in 60 files · static e2e 115 passed / 84 skipped / 0 failed.** Three runs total across the order: run 1 red on two specs with its own cause printed (`the tree changed during verify:all` — a prettier line-ending fix landed mid-run; both passed solo), run 2 green on a frozen tree, run 3 green after the copy edit.
- **PROVEN BY UNIT AND STATIC ONLY.** All three fixes — the canonical-row read, the combined cap of 40, and the honest save — are proven by 683 unit tests and the 115-test static suite and **NOTHING ELSE**. No live round, no gate run, no probe against the deployed API ran in this order (the standing rule, 2026-09-13). **The next testing session the founder names is what proves them against the deployed API**, and `live-brand-rules.spec.ts` and `live-brand.spec.ts` both walk this screen and neither was touched.
- Next: item 66 is the founder's — a reviewed in-product merge, or Hasan making the voice name unique per org (the real fix, and the one that removes the class). Item 64 stands, still deliberately unfixed.

### 2026-09-14 11:20 — ORDER UX-0913 Phase 0 — INVENTORY taken, HELD at the Phase 3 stop clause (report-and-stop, zero code)

- Did: **Phase 0 only. No code changed; `feat/ux-0913` is branched off `origin/main` = `0482f20` and is CLEAN.** The inventory found every string the order names, and then found that **Phase 3's premise does not hold in this repo** — one of Phase 0's four stop conditions ("a theme token has no clear mapping in Phase 3"), so the order is HELD before any edit.
  **(a) The six OLD strings: all six found, all unambiguous.** Four live in `src/lib/messages.ts` (the central copy module), one in `organization-screen.tsx` ×2, one in `routes.tsx`.
  **(b) Coffee residue: 12 hits in scope, every one a `placeholder` attribute, across 6 files.** NONE is real org data on the wire; none is a wizard seed (the wizard was deleted by ONB-0827 P2, and `RootGate` sends a session with no workspace to `EmptyOrgScreen`). Everything else — ~150 hits — is the static demo dataset under `src/data/**` (out of scope by the order) or e2e fixture data.
  **(c) Hardcoded colour in app surfaces: effectively ZERO already.** `#FF1E57` / `#B1204A` are retired everywhere (design.md 1.1) — the only survivors are one explanatory comment in `tokens.test.ts` and the historical docs. Zero Tailwind arbitrary colour classes anywhere. The only hex outside the token files is inside recharts *selector* strings in `src/components/ui/chart.tsx` (`[&_.recharts-…[stroke='#ccc']]`), which are selectors, not declarations, and that file is never hand-edited (CLAUDE.md rule 3).
  **(d) Fonts DIFFER: app = Inter (vendored variable, self-hosted); marketing = DM Sans + IBM Plex Sans Arabic.** Not "no change".
  **(e) Landing = `RootGate` in `src/routes.tsx:138-144`, which renders the Dashboard at `/`. Today in LIVE mode is REAL** — `live-today.tsx` derives the queue from the proposals ledger on the wire (D-INT-J), not from demo data. **Phase 2's precondition is MET.**
  **(f) `previewTone` (`src/data/brand.ts:253`) is ALREADY a real generate call in live mode** — `POST /orgs/:id/alphastudio/posts/tones-preview` — falling back to the composed line when the run returns no sample; static mode composes. Nothing to build; reported as asked.
  **(g) 51 assertions across 32 files** assert `heading "Dashboard"` as the post-login landing (24 of them live spec files), plus the trap-22 tripwire in `e2e/global-setup.ts` whose error text names that symptom.
- Phase: post-W7 / INT — a UX order, no phase advanced
- Files: none (branch created only)
- Decisions: none logged — D-UX-0913-A/B/C are NOT written, because A rests on the premise this hold disputes
- Verify: **not run.** No lint/typecheck/unit/build either: nothing was edited. **No Playwright of any kind was started** — the order's hard rule (port 5199, trap 22) stands until the founder says the GATE-0910 proof is finished; no node process was running on this machine when checked.
- Next: the founder's ruling on the three collisions in the report — (1) Phase 3's palette source of truth, (2) the Interactive-Gold-as-primary contrast arithmetic, which is 4.26:1 at its best and cannot meet the order's own 4.5:1 bar, (3) Phase 2's blast radius of 51 assertions / 32 files and the Dashboard route collision at `/`. Phase 1 (copy + placeholders) is unblocked and fully inventoried, and can ship the moment the founder says so.

### 2026-09-14 11:55 — ORDER UX-0913/P1 — Abdallah's copy pass BUILT; 15 placeholders neutralised and a guard that keeps them out

- Did: **The copy phase, and only it.** Phase 3 was withdrawn by the founder on the Phase 0 report and Phase 2 deferred; both reasons are recorded in decisions.md as D-UX-0913-C, because they are design law rather than scheduling.
  **Six exact replacements** at the lines Phase 0 named. **Six new lines** — the Brand Voice intro, the Sources/Topics line, the topic helper, the Knowledge subtitle, and the Do and Don't helpers. **The three drafting-model descriptions** rewritten to say what the model does rather than how it behaves (they render on the schedule's "Generation model" field, which is where that picker actually lives — Generate has no model picker). **Generate and Studio** each given a self-explanatory header subtitle, because the nav has NO secondary-text slot: its only tooltip repeats the label and appears solely when the rail is collapsed, so the order's own fallback — the page header subtitle — is what carries it.
  **15 placeholders neutralised**: the order's 12, plus three I flagged rather than left — the Don't rule's "Call anything artisanal" (direct sibling of a Do placeholder I was already changing; leaving it made the pair incoherent), the knowledge description's "Our spring price list, valid until June" (the founder's own example list names this field and its replacement, though Phase 0 had not listed the file because it carried no coffee string), and an offer placeholder the field never had at all.
  **`brandExamplesPending` DELETED** with both consumers (`brand-voice-screen.tsx`, `tone-editor.tsx`): the example editor is now simply absent in live mode and says nothing about why — the order's "no explanation text at all".
  **The four honest-limit strings REWORDED, not deleted** (item 6). Each still states its limit truthfully in customer language; "backend phase", "later phase" and "on the wire" are gone. **None needed a wire change, so this order files NO open item.**
  **TWO DEVIATIONS, both forced by house law, both reported.** (1) The founder's example placeholder `https://example.com/blog` is ILLEGAL here — `ab/no-network` forbids an `http(s)://` literal anywhere in `src/` (CLAUDE.md rule 2) and lint caught it — so the three URL placeholders are scheme-less (`example.com/blog`, `/guide`, `/article`), which is also exactly what `normalizeSourceUrl` stores. (2) The new copy was asked for "in messages.ts alongside its neighbours"; measured, the neighbours are NOT there — `messages.ts` holds notices/errors/empty-states, and all 19 field `description=` props and every `routes.tsx` handle context are inline, with `routes.tsx` importing MESSAGES nowhere. So the page subtitles went inline in `routes.tsx` (matching the order's own item-1 instruction for the Tones context) and the field helpers inline beside their 19 siblings.
  **New guard `src/lib/placeholder-law.test.ts`** — 11 tests. It sweeps every customer-facing surface, not just `placeholder=`, because borrowed sample content is equally wrong in a helper, a default or a seeded row; `src/data/**` is excluded on purpose (the demo world IS Atlas Roasters). **PROVEN, not assumed: an injected `placeholder="single origin"` was caught with its file:line, then reverted.**
- Phase: post-W7 / INT — a UX order, no phase advanced
- Files: `src/lib/messages.ts`, `src/routes.tsx`, `src/lib/placeholder-law.test.ts` (new), `src/data/entities/generation-models.ts`, `src/features/settings/{organization-screen,brand-voice-screen,tone-editor,sources-screen,live-knowledge,knowledge-upload-form}.tsx`, `src/features/generate/generate-screen.tsx`, `src/features/studio/studio-screens.tsx`, `src/features/auth/signup-screen.tsx`, `src/features/system/empty-org-screen.tsx`, `e2e/{hsn-series,live-brand,live-create-visual}.spec.ts`, `.agent/{decisions,state,sessions}.md`
- Decisions: see decisions.md — **D-UX-0913-C** (placeholders describe the input), plus the recorded reasons Phase 3 was withdrawn (Part 1 governs the app, Part 7 the visitor world, and `primary = Interactive Gold` measures 4.26:1 against its own 4.5:1 bar) and Phase 2 deferred (Dashboard has no route of its own; 51 assertions in 32 files)
- Verify: **lint · typecheck · guard-static 355 · unit 694/694 in 61 files · build — GREEN, and run TWICE, the second pass on the frozen tree after prettier.** Unit went 683/60 → 694/61; the +11 is exactly the new guard, and **no existing test broke**, so nothing in the unit suite had been asserting the old copy.
- **NO PLAYWRIGHT RAN, of any kind** — the founder's word is still pending on the GATE-0910 proof (port 5199, trap 22). **Three spec files are EDITED BUT UNRUN**, and that is this order's one loose end: `hsn-series.spec.ts` (static, 1 assertion), `live-brand.spec.ts` (3) and `live-create-visual.spec.ts` (1). They were the only specs anywhere in `e2e/` asserting a replaced string; the two that asserted the now-deleted notice assert the example editor's ABSENCE instead. **Screenshots NOT taken — they need Playwright, so before AND after both wait on the same word.**
- Next: the founder's word to run the static suite and take the screenshots, then the push. Phase 2 waits on the Dashboard-route ruling; the dark repalette comes as its own order.

### 2026-09-14 12:40 — ORDER UX-0913/P1-R — the copy moved to the control it describes, and the label collision written down four ways

- Did: **The founder's rider was right and the P1 report was wrong in exactly the way it documented.** P1 noted "this picker lives on the schedule's Generation model field, not on Generate" and filed it as a location note. It was the blocker. The three rewritten descriptions had landed on `generation-models.ts`, whose only rendering surface is the schedule's `modelAlias` control — on-demand grounding copy on a scheduling control, and false at the wire: "Grounded in your knowledge and sources" had come to describe `gm_creative` → **`fast`**, and "Stays closest to your approved knowledge and sources" → `gm_precise` → **`quality`**. Neither alias is a grounding mode.
  **1. Reverted byte-for-byte**, proven against the P1 diff: `gm_balanced` "Reliable everyday drafts — the sensible default.", `gm_creative` "Bolder angles and fresher hooks; a touch slower.", `gm_precise` "Tightest grounding to your sources; best for regulated topics."
  **2. Rewritten to `modelAlias`'s own truth.** `gm_balanced` → "The default. Every plan can use it." — both halves checkable in code (it is the fallback in BOTH directions, `scheduling-adapter.ts` `?? 'gm_balanced'` and `scheduling.ts` `?? 'balanced'`; tier `free`). `gm_precise` → "Our highest-quality drafting model." — the confirmed alias is literally `quality`, which is the only evidence there is and is enough for that sentence and nothing more. **`gm_creative` LEFT EXACTLY AS IT WAS**, per the founder's constraint: `gm_creative → fast` is unconfirmed by elimination (open-items 9), so any behavioural sentence could become false the day Hasan answers. Worth recording that the reverted line shows the tension itself — "a touch slower" sits oddly beside an alias named `fast`.
  **3. The real control, balanced only.** `PLANS` in `live-generate.tsx` takes Abdallah's line for `balanced`. `creative` and `precise` are FROZEN under **new open-items 67**, with all three current strings recorded verbatim so Hasan's answer can be checked against them. The conflict is real and unresolvable from here: "Grounded in a curated web search instead" is a grounding SOURCE claim, "leans less on verbatim facts" is a grounding STRICTNESS claim, and we do not replace one possibly-wrong claim with another.
  **4. Four artefacts, because a decisions entry alone would not have stopped this.** D-UX-0913-D; a cross-pointing ⚠️ warning at the head of each table (each names the other's file, wire field and values); the `ApiPlan` doc comment in `api/types.ts` extended to name the **label** collision and not only the value incompatibility; and `generation-models.test.ts`, which fails if grounding vocabulary reappears in a schedule description — and prints the offending row's wire alias, so the next reader learns the distinction from the error itself.
- Phase: post-W7 / INT — a correction rider, no phase advanced
- Files: `src/data/entities/generation-models.ts`, `src/data/entities/generation-models.test.ts` (new), `src/features/generate/live-generate.tsx`, `src/api/types.ts`, `.agent/{decisions,open-items,state,sessions}.md`
- Decisions: see decisions.md — **D-UX-0913-D** (two vocabularies, identical labels; name the wire field before editing an option description). It does not replace D-INT-D — D-INT-D was about values never being mapped, and that held; this is about the labels being identical, which nobody had written down.
- Verify: **lint · typecheck · guard-static 356 · unit 703/703 in 62 files · build — GREEN.** Unit went 694/61 → 703/62; the +9 is exactly the new guard. **The guard was PROVEN BY BREAKING IT:** the exact string the founder caught was re-injected and it failed on both "knowledge" and "sources", naming `gm_precise → modelAlias 'quality'` in the message, then reverted.
- **NO PLAYWRIGHT RAN** (the founder's word on GATE-0910 is still pending). **No spec asserts a description or hint from EITHER table** — all 13 strings checked. `live-scheduling.spec.ts:137` and `live-schedule-repair.spec.ts:118` match `/^Balanced/`, which anchors on the model's `name`; only `description` changed, so both still match exactly as before. `studio-capabilities.spec.ts:187,200` match `/^Precise/` and `/^Creative/` on the Studio's MEDIA plan buttons — the `ApiPlan` vocabulary, untouched by this order.
- **PUSHED** `feat/ux-0913` to origin on the founder's word, so the theme order can branch off it. **NOT merged.**
- Next: Hasan's answer on open-items 67 unfreezes creative and precise; open-items 9 (`gm_creative → fast`) unfreezes the schedule's Creative description. The static suite and the before/after screenshots still wait on the founder's word that the GATE-0910 proof is finished.

### 2026-09-14 14:10 — ORDER THEME-0913 Phase 0 — the reference read and measured, the system proposed, nothing built

- Did: **Phase 0 only — read-only, no source touched.** Branch `feat/theme-0913` confirmed at `bb83ffe` (= `origin/feat/ux-0913`), clean tree, not yet on origin.
  **The reference measured, not summarised.** `marketing.css` read verbatim; every Part 7 token converted to OKLCH and every pair contrast-measured with the same math `tokens.test.ts` uses. **Part 7's prose is wrong on one factual point: the surfaces are NOT "warm-leaning".** All six measure hue **239–244° (blue)**; only the text tiers are warm (h≈72°). The order's design law says "one warm graphite hue across all four", so this is a genuine conflict between the order and the measured reference — **raised, not resolved.**
  **The four-step ladder needs no invented value.** Part 7's six surfaces re-role exactly: sunken `#05080b` · canvas `#080d11` · raised `#0c1217` · overlay `#161f26`, with `#10171c` and `#1d272f` as the hover steps. ΔL +0.0239 / +0.0228 / +0.0339 between the roles, strictly monotonic.
  **Two roles Part 7 never needed, both measured into place.** Error — a cooler crimson at **`#ed647c`** (OKLCH 0.68/0.17/12), **20.6° from the accent** in hue, clearing 4.5:1 on all six surfaces and on the `/10` badge tint. Warning — **`#ea9e51`**, 31.2° from the accent. **The form-boundary gap closed with a value already in the palette:** solving `--input` for ≥3:1 on all six surfaces landed on **`#857f79`, which IS Part 7's `--c-text-3`** — Part 7's own hairlines (white 7/12/20%) top out at **1.84:1** and cannot legally identify a control.
  **The prior withdrawal does not repeat.** D-UX-0913-C killed the last palette attempt partly because `primary = Interactive Gold` measured 4.26:1. This proposal never makes gold interactive — the new design law forbids it — so that arithmetic does not apply.
  **Inventory:** app is unusually token-clean — **zero** raw Tailwind palette colours and **zero** stray hex in `src/features/**` and `src/components/ab/**`; the only hex outside the two token files is `chart.tsx`'s recharts selector strings (rule-3 protected). Light-surface assumptions are narrow: 4 × `bg-black/10` overlay scrims, 8 shadow-based elevations, `motion.tsx`'s shadow. **64 accent-fill buttons** across 229 (Today: 10). **35 physical-direction classes, 0 logical** — and the app has **no `dir`/locale mechanism at all** and **zero Arabic UI strings** (Arabic exists only in `features/marketing/**`).
- Phase: post-W7 / INT — a theme order, no phase advanced
- Files: **none** (`.agent/sessions.md` only)
- Decisions: **none ratified.** D-THEME-0913-A…F are proposed in the Phase 0 report and await the founder's word; nothing written to `decisions.md`, `design.md` or `CLAUDE.md` yet.
- Verify: **not run** — Phase 0 builds nothing, so there is nothing to check. Playwright remains frozen pending the GATE-0910 word.
- Next: the founder's ruling on the four blockers in the report — the warm/cool hue conflict, the Arabic-screenshot scope gap, the theme-toggle retirement (breaks `design-layer.spec.ts` + `smoke.spec.ts`), and the Inter→DM Sans switch (breaks `smoke.spec.ts:34`). Then Phase 1: rewrite `tokens.css` + `tokens.test.ts`, re-theme Today.

### 2026-09-14 15:05 — ORDER THEME-0913 Phase 1 — one theme, the ladder, and Today's most important sentence

- Did: **Phase 0 accepted in full; two of its measurements changed the order.** The founder amended his own design law from "one **warm** graphite hue" to the measured hue after Phase 0 proved Part 7's "warm-leaning" prose wrong (all six surfaces 239–244°, i.e. blue; the warmth is in the ink at h≈72°), and ruled Arabic out of scope with an escalated open item.
  **`tokens.css` REWRITTEN, not patched.** Dark is the product; the light palette is retired with no toggle. Values are **hex, not OKLCH**, so the file can be diffed against `marketing.css` by eye. Four-step ladder from Part 7's own six values — sunken `#05080b` < canvas `#080d11` < card `#0c1217` < popover `#161f26` (+ two hover steps) — strictly monotonic. Accent `#ff4e2d` is the one action colour; gold `#e3c084` is quiet metal.
  **`tokens.test.ts` REWRITTEN.** Every premise of the old file was void: it read a `.dark` block that no longer exists, matched `oklch()` only, and defended a light-surface split for a "signature pink" gone two orders ago. New matrix: text on every surface it may appear on, the `--subtle-foreground` bar asserted in **both directions** (clears AA on page/card, and STILL FAILS on the overlay steps — if it ever passed everywhere the ladder would have flattened), the badge `/10` pattern, `--input`/`--ring` at 3:1, the monotonic ladder, the chart ramp, and hue distance as **circular** distance — **the raw-subtraction bug found in Phase 0 is fixed, not carried forward**.
  **NEW `one-theme.test.ts` (RULING 4)** — 19 shared values compared by VALUE, since the two files name things differently on purpose. **PROVEN BY BREAKING IT:** `--c-accent` in `marketing.css` drifted `#ff4e2d` → `#ff4e2e`, the guard failed naming both tokens and both values, then reverted — **`marketing.css` is byte-for-byte unchanged** (`git diff --quiet` confirmed).
  **THREE UNLAYERED OVERRIDES, each forced by arithmetic, none a `ui/` hand-edit.** (1) Form controls onto the sunken step: `dark:bg-input/30` composites OVER the parent and **can never be darker than it** — solving `0.3·I + 0.7·card = sunken` gives a negative channel, so no token can do it. (2) The scrim: measured, darkening moves modal separation only **1.18:1 → 1.24:1 at ANY alpha** because the canvas is already near-black; what a scrim CAN do is suppress, and at 72% the page text behind drops **13.1:1 → 2.0:1**. (3) The destructive plate pinned to 10%: shadcn's `dark:bg-destructive/20` measures **4.06:1 over `--popover`**, under AA — the colour is the approved one, so the **bump** is what changed.
  **TODAY re-themed, with §5.1 pulled forward** because a stubbed empty state would make Abdallah's review meaningless. The tracked-out uppercase eyebrow is **deleted**. `"No drafts yet"` was a defect by the law — it named neither what would appear, nor why nothing had, nor what to do — and is now **three honest branches** (needs setup / waiting, with the real time / no rhythm set), destinations unchanged, copy in `MESSAGES.empty.*`. The draft-card asset tile lost its gold fill (gold is not a placeholder).
  **RTL in structure (RULING 2):** all **35** physical-direction classes **plus 7 negative margins** the first sweep's regex missed → logical properties. The app hardcodes left or right **nowhere**. **Inter RETIRED** (D-THEME-0913-F): DM Sans + IBM Plex Sans Arabic, no new dependency, vendored woff2 deleted, build confirms Inter is out of the bundle.
  **CRITIQUE, and the one element removed.** Does it read as the same product as the website? Yes, and by construction rather than resemblance — every surface, text tier, accent, gold and easing is the website's own value, guarded. What would any dark SaaS template have produced? Two things, both caught and replaced: the five-hue categorical chart ramp (Phase 0, replaced with a ramp that invents no hue) and the reflex to reach for a heavier shadow under modals (replaced by the measured scrim, since shadow cannot separate anything here). **Removed: `--line-quiet` and `--line-strong`** — two hairline tiers copied from Part 7 that had **zero consumers**, which is precisely the fault D-THEME-0913-G names. A palette is not a place to store colours nobody asked for; the guard rows went with them (752 → 750 tests).
- Phase: post-W7 / INT — THEME-0913 Phase 1 of 5; Phases 2–5 not started
- Files: `src/styles/{tokens.css,globals.css,tokens.test.ts,one-theme.test.ts}` (last is new), `src/lib/{theme.tsx,messages.ts}`, `src/components/ab/{app-shell,motion}.tsx`, `src/components/ab/theme-toggle.tsx` (**deleted**), `src/styles/fonts/` (**deleted**), `src/features/today/{today-screen,draft-card,draft-detail-screen}.tsx`, `src/features/system/not-found.tsx`, `src/features/dev/{dev-kitchen-sink,dev-states}.tsx`, plus 14 files touched only by the RTL conversion, `design.md` (Part 1 rewritten, Part 2 settled, §7.1 corrected, header rewritten), `CLAUDE.md`, `.agent/{decisions,state,open-items,sessions}.md`
- Decisions: see decisions.md — **D-THEME-0913-A** (one theme; the Part 1/Part 7 separation repealed; the §7.1 prose defect), **-B** (dark replaces light, no toggle), **-C** (the accent rationed), **-D** (error leaves red at a measured 20.6°, recorded as a deliberate compromise so it is not "fixed" later), **-E** (elevation is lightness; the scrim measurement), **-F** (DM Sans, superseding Part 2's Inter-pending), **-G** (the chart ramp invents no hue — one chart, one series). New **open-items 68**: no Arabic UI locale, no `dir` mechanism, in an Arabic-first product.
- Verify: **lint · typecheck · guard-static 356 files clean · unit 750/750 in 63 files · build — GREEN, run TWICE, the second pass on the frozen formatted tree.** Unit went 703/62 → 750/63; the +47 is the rewritten `tokens.test.ts` and the new `one-theme.test.ts`, and **no existing test broke**. Prettier was run on **only the four files this order made dirty** — the tree's pre-existing 17-file baseline was measured at HEAD and is unchanged (no drive-by reformatting).
- **NO PLAYWRIGHT RAN, of any kind.** **Three specs are KNOWN casualties and were deliberately left UNEDITED** per RULING 3: `design-layer.spec.ts:95–118` and `smoke.spec.ts:37–39` (the deleted theme toggle) and `smoke.spec.ts:34` (asserts the body font contains "Inter"). **Screenshots are the single outstanding item** — Today in four states, desktop and mobile, English only, under `Docs/qa/theme-0913/`.
- Next: **Abdallah sees Today. Nothing in Phase 2 starts until the founder says so.** When Playwright is cleared: take the screenshots and edit the three specs, reported together.

### 2026-09-14 16:40 — ORDER THEME-0913 Phases 2–5 — the chrome, the components, the sweep, and the warmth pass

- Did: **Phase 1 accepted; Phases 2–5 built without stopping, one report at the end.** Founder review of Today on the preview PASSED and the colour direction is settled.
  **THE THREE PREVIEW FINDINGS.** (1) **The login split panel** — it was `bg-primary`, a charcoal fill for as long as `--primary` meant charcoal, which silently became half a viewport of `#ff4e2d` when the accent took that token. **Decided deliberately: graphite** (D-THEME-0913-H). Not for contrast — dark ink on the accent measures **5.86:1** and passes — but because half a screen of the action colour teaches a user on their first screen that orange means nothing, next to a sign-in button that is also accent. Panel copy measured on the new ground: heading **11.16:1**, body **5.84:1** worst case over the gold bloom, which drops from 70% to **18%** (at 70% the body would have been **1.46:1**). The accent survives as one character — the full stop closing the headline, the website's own idiom; copy unchanged, since all three headings already ended in a period. (2) **The Dashboard tile is a REAL STATE, not an accidental tint** — `StatCard tone="warning"` at `credits < 50`, and it is `--warning` (h 63.9°), not the accent. The rule is uniform and available to every tile; two tiles pass no tone because no warning condition is meaningful for them, which is a product fact, not a theme one. (3) **Font confirmed from the built artifact, not the config**: `html{font-family:DM Sans Variable,…}`, `.font-display{font-family:var(--font-sans)}` → the same stack, `--default-font-family` also resolves to DM Sans, and **`grep -c Inter` on the built CSS returns 0**. A browser-computed read still needs Playwright and is named as outstanding.
  **PHASE 2 — CHROME.** Sidebar active was a **filled block identical to hover** (both `bg-sidebar-accent`); it is now a gold inline-start rule + full-strength text, hover keeps the surface step, and a row that is both shows both (D-THEME-0913-I). Settings sub-nav had the same fault **inverted** — hover was LIGHTER than selected — and takes a gold underline. Chrome (top bar + rail) moved to the raised step so the work surface is the darkest thing on screen. **The `eyebrow` PROP was deleted, not just its one call site** — an affordance that still exists comes back — and with it all **7** remaining tracked-uppercase labels product-wide, including an all-caps "BRAND KIT" role badge.
  **PHASE 3 — COMPONENTS.** Four new unlayered overrides, each forced by an assumption no token can reach: **tooltips** shipped `bg-foreground text-background`, i.e. a near-white card over graphite — the one genuine orphan light surface in the product; **secondary buttons** gained the border the law requires; **tables** gained zebra-by-surface-step, a sticky header and tabular figures, none of which were in the primitive. **`chart.tsx` re-themes fully through `--chart-1`** — its `[stroke='#ccc']`/`[stroke='#fff']` strings are attribute selectors matching recharts' OWN defaults, not colours we set. **One thing is genuinely unreachable and is reported, not hacked:** its tooltip hardcodes `bg-background`, so it sits one step BELOW the card it floats over. Reaching it would mean a new selector depending on recharts internals — the exact fragility rule 3 exists to prevent.
  **PHASE 4 — SWEEP.** Accent counted per file: **62 default-variant Buttons**, and most multi-count files are **mutually exclusive branches**, not collisions (reset-password's 5 are five separate `return`s; generate's 3 are three player phases; tones' 2 are the empty and populated screens). **One genuine collision found and fixed:** the status machine allows `publish_failed → scheduled`, so a failed card set `canSchedule` AND `failed` and rendered **two accent buttons side by side**; Schedule now steps down, because on a post that just failed Retry is the primary. One accent-filled `<Badge>` ("Invited") became gold — a status badge, never the action colour. Zero default Badges remain. The 35 physical-direction classes were already logical from Phase 1.
  **PHASE 5 — WARMTH.** §5.1: **6 EmptyStates had no action**; three got a real control (Studio→"Open the studio", an empty FILTER→"Show all tools", analytics→"Review today's queue"), and three are documented as control-adjacent — the form that fills them is 40px above, so a button would duplicate a visible control. The dashboard feed's "Nothing here yet" became "Approvals and alerts land here", with **no action on purpose**: a notification feed is filled by the product working, not by pressing something. §5.2: **EmptyOrgScreen rebuilt** — wordmark instead of a bare gold square, then the three things Malaky actually does in **Today's own vocabulary**, then the one field. Warmth is concrete, not cheerful: no exclamation marks, and the failure is still named honestly. §5.3: **SaveBar now says "Saved" at the bar that was pressed** before it leaves — read from `dirty` going true→false, so it can never claim a success that did not happen. §5.4: `SkeletonShell` **escalates after 10s** with one honest line about the server waking — the founder's measured **12–23s cold-start tail** is a real experience, and it is a sentence, not a fake bar. §5.5: `ErrorState`'s default title was **"Something went wrong" on 16 of 23 call sites** — an apology stacked on a message that already explained itself; it is `"This didn't load"` now, and a `reference` slot renders the request id **last and quietest**. §5.6: **the biggest diff, and it was not colour** — the top-level section gap measured anywhere from **4px to 32px** across 25 screens; 32px (Today's, the approved reference) is now applied **once in AppShell's `<main>`**, and the scale is normalised to one ladder with **62 off-scale gaps** converted and nothing off-scale left. §5.7: **the one moment is finishing the queue** — a single gold rule drawing once when the LAST draft is approved, as a third `data-ab-motion` value so it inherits the reduced-motion guarantee Playwright already asserts, and firing only on a TRANSITION so an already-empty queue never claims a finish.
  **CRITIQUE — the element removed:** the active sidebar icon was also coloured gold, making **three signals for one fact** (rule + text weight + icon). The rule carries the collapsed rail on its own, so the icon colour went.
- Phase: post-W7 / INT — THEME-0913 Phases 2–5, the order complete
- Files: `src/styles/{globals.css,tokens.css}`, `src/components/ab/{app-shell,page-header,save-bar,skeletons,error-state,error-state.test,motion}.tsx`, `src/components/ab/theme-toggle.tsx` (deleted, P1), `src/features/auth/auth-layout.tsx`, `src/features/settings/{settings-layout,team-screen,media-files-section}.tsx`, `src/features/system/empty-org-screen.tsx`, `src/features/today/{today-screen,draft-card,draft-detail-screen}.tsx`, `src/features/studio/{live-jobs,capability-grid,studio-screens}.tsx`, `src/features/{analytics,dashboard,billing,calendar,dev}/…`, `src/lib/messages.ts`, `.agent/{decisions,state,sessions}.md`
- Decisions: see decisions.md — **D-THEME-0913-H** (auth panel graphite, with the measurement that says it was not an a11y fix), **-I** (gold rule as the nav indicator; the settings sub-nav inversion), **-J** (section rhythm in the shell, not per screen), **-K** (the memorable moment, and why that place)
- Verify: **lint · typecheck · guard-static 356 files clean · unit 750/750 in 63 files · build — GREEN, run twice, the second pass on the frozen formatted tree.** Prettier run on **only the 9 files this order made dirty across all phases**; the tree's pre-existing 17-file baseline was measured at HEAD and is unchanged.
- **NO PLAYWRIGHT RAN.** The three held specs stay **unedited** per RULING 3: `design-layer.spec.ts:95–118`, `smoke.spec.ts:37–39`, `smoke.spec.ts:34`. **Screenshots remain the one outstanding item.**
- Next: the founder's word on the GATE-0910 proof unfreezes the screenshots and the three specs. Nothing else is open in this order.

### 2026-09-14 17:55 — ORDER THEME-0913 close-out — Playwright cleared, the suite run, the shots taken, both orders closed

- Did: **The lane was free before anything touched Playwright, and I checked rather than assumed** — 0 `node.exe`, 0 processes under `ms-playwright`, 0 TCP listeners on 5199. The only process matching "playwright" was the PowerShell probe matching its own command line. **This morning's 05:38–05:41Z activity was `verify:all`, NOT the proof** — `verify.json` is written by `verify-all.ts`, it reports `ok: true`, and it produced no `summary.json`, no `.gate/runs/` entry and no live rounds; its `guard-static` says **354 files** against 356 now, so it ran on the pre-THEME-0913 tree.
  **THE THREE HELD SPECS, updated to what the product IS.** `smoke.spec.ts:34` asserted the body font contains "Inter" → now asserts **DM Sans** and additionally that Inter is absent (D-THEME-0913-F). `smoke.spec.ts:37–39` pressed the deleted toggle → now asserts the `dark` class is present **without an interaction** and that no toggle has come back (`toHaveCount(0)`). `design-layer.spec.ts:95–118` scanned light, pressed the toggle, scanned dark → those two scans would now be the same scan twice, so it is **one axe scan** plus the same no-interaction assertion; the test is renamed "kitchen sink is clean in the one theme". **Two further readiness gates** in the reduced-motion test used the toggle merely as "is the page up" and were re-pointed at the Kitchen sink heading.
  **STATIC E2E — three full runs, and the discipline mattered.** Run 1: **114 passed / 1 failed / 84 skipped**. The one red was `hsn-0902.spec.ts:102`, asserting `getByText('brand kit', { exact: true })` — **a spec asserting something this series deliberately changed**: the badge was a lowercase source string that CSS rendered as "BRAND KIT", and Phase 2 banned all-caps labels, so the source became sentence case. Run 2 after the fix: **5 failed**, which I did NOT paper over — three `entry-flow` tests and one `hsn-series` test that had **passed in run 1 on identical application code**, plus `hsn-0902` still red for a NEW reason (my first fix made the badge text identical to the file's own name, so the locator matched two elements). **Diagnosed rather than adjusted:** the badge got a real hook (`data-slot="file-role-badge"`) and the assertion now proves its WORDS (`toHaveText('Brand kit')`) rather than its case; the other four were re-run **serially in isolation and all 16 passed**, confirming parallel-load flakes, not regressions. Run 3, authoritative: **115 passed / 0 failed / 85 skipped.**
  **SCREENSHOTS — and four bad frames caught before they reached a review.** Today in all four states × desktop + mobile, English only, **under `Docs/qa/theme-0913/`, never `test-results/`**. The first spec waited on `aria-busy` and `role="alert"` and produced a **skeleton for the "default" frame and a bare route spinner for "error"**. Three real causes, all fixed: the `/dev/states` controls are `role="radio"` with the note in their accessible name; in the forced-error world the **Dashboard renders the same ErrorState title**, so the marker was visible before navigation finished (fixed with `waitForURL`); and a bare `[aria-busy]` also matches the **route-level Suspense fallback**, so the loading marker is now Today's own named skeleton. Every frame re-asserts its marker at the shutter. All eight verified by eye.
  **THE FIVE M-BIL-1 FRAMES ARE FILED, NOT REPLACED (open-items 69).** They are not stale design references — they are the **evidence record of a payment event** (Stripe test mode, real orgs 1813/1814), and that folder's own rule is that the Stripe pages are driven once and nothing that can bill is retried. Re-shooting them in the dark theme would produce pictures of a run that never happened, which is worse than a stale record. The README carries the same note inline.
- Phase: post-W7 / INT — THEME-0913 and UX-0913 both CLOSED
- Files: `e2e/{smoke,design-layer,hsn-0902,live-brand-kit}.spec.ts`, `e2e/theme-shots.spec.ts` (new), `src/features/settings/media-files-section.tsx`, `Docs/qa/theme-0913/*.png` (8, new), `Docs/qa/m-bil-1/README.md`, `.agent/{open-items,state,sessions}.md`
- Decisions: **none new.** Every entry both series call for is filed and verified present as a heading: **D-UX-0913-C**, **D-UX-0913-D**, and **D-THEME-0913-A through -K**. (D-UX-0913-A/B were deliberately never written — sessions.md records that A rested on a premise the hold disputed — so C is where that series starts.) **Recorded for the record: BOTH GATE-0910 proof runs (`20260913-062802`, `20260913-070254`) carry `"dirty": true`** — commit `8c0694a` alone does not reproduce them.
