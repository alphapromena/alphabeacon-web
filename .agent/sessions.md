...
### 2026-08-27 07:45 — PROBE-INT13 control call: the route is healthy, the `posts[]` path is not

- Did: on the founder's explicit authorization, sent **one** `media.generate` single-job POST to `/orgs/948/alphastudio/media/jobs` — same route, same org, same `plan: balanced`, same `params` (1:1, png) as the fan-out sends, differing **only** in the body shape. **202 in 4333 ms**, req `ea5b1dc5-24bb-4909-aa49-1fdc65dd5acd`, job `mjob_881107b9b261e689486e9da0` → `succeeded` with a 1024×1024 PNG; wallet 4993 → 4990 (**3 cents**). That converts verdict 5 of the probe from inference to observation: the route, capability resolution, renderer and wallet are all healthy, and **only Hasan's `posts[]` fan-out fails to return a receipt**. Two bonus facts the control pinned down, both of which Phase B needs: the **single-job** receipt is the full job object (`status: "queued"`, `assets: []`, `modelAlias` already resolved) exactly as `MediaJobReceipt = ApiMediaJob` records, and `origin` is **echoed as sent** here (`{"kind":"standalone"}`) where the fan-out **derives** `{"kind":"linked","ref":…}` — confirming `kind` as the Studio-vs-post-linked discriminator. The fan-out path was not touched and nothing was retried under any status, per the order. Caveat recorded in the doc rather than hidden: the control ran ~17.6 h after the fan-out sends, since the fan-out cannot be re-sent to close the gap (it bills on every attempt).
- Phase: INT-13 Phase A (probe) — Phase B still NOT started, still blocked on the unobserved fan-out receipt
- Files: `Docs/api/probe-int13-social-posts-media-2026-08-26.md` (control section appended; verdict 5, the cost line, the call table and the Ward question corrected so the doc no longer contradicts itself), `.agent/open-items.md` (item 34a), `.agent/sessions.md`
- Decisions: none — the control settles a fact, not a design choice
- Verify: not run (no product code changed)
- Cost: **3 cents**, as authorized. Running total for PROBE-INT13: 10 cents on QA org 948
- Next: unchanged — Ward still has to say **what the fan-out's success body is meant to be**. The control makes the question sharper, not answered: ask whether the proxy validates the upstream's fan-out list against the single-job schema, since a single-job body is demonstrably unaffected
