...
function e2eNavigationRuleHolds(): boolean {
  ...
  const specs = ['entry-flow.spec.ts', 'today-queue.spec.ts', 'calendar-connections.spec.ts']
  let ok = true
...
function main(): void {
  ...
  if (rerun) {
    ...
    const suite = suiteRowsFromReport({
      e2eLabel: 'e2e (calendar, connections, axe)',
      needE2e: !skipE2e,
      e2eMustPass: [/calendar/i, /connection/i],
    })
