...
**Verify:** every A/M state renders per screens4 across the dataset + state switchers · Playwright golden: signup → verify → onboard → dashboard · ~~marketing pricing and H1 pricing assert equal from one module~~ **superseded by D-M2-B**: `verify:w02` asserts the SEPARATION instead — nothing under `features/marketing/` imports `@/data` (one exception, the layout's session read) · `posts_per_day=4` blocked with the named message · axe + reduced-motion clean, on all five marketing routes at 1440 and 390.
