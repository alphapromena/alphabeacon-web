...
- **Datasets replace scenarios.** `src/data/datasets/` composes whole-tenant states — _visitor_ (signed out), _fresh org_, _active org_, _low credits_, _needs re-auth_, _past due_. One tap at `/dev/datasets` swaps the dataset the whole app reads. Datasets are the only way a screen reaches a different world. (A _heavy history_ world was planned and has not been needed; add it in W7 if the bundle or list-rendering work calls for one.)
...
│  │   marketing/ auth/ dashboard/ today/ drafts/ calendar/
...
- `src/styles/tokens.css` defines the shadcn variable set (`--background`, `--foreground`, `--primary`, `--destructive`, `--muted`, `--border`, `--radius`, chart colors…) in **OKLCH**, light + dark, valued from `design.md`'s palette; the app is light+dark. ~~marketing is light-only~~ — **amended 2026-08-23 (M2):** the visitor world has its own token file, `src/styles/marketing.css`, scoped to `html[data-mk-world]` / `.mk-world`, and it is **dark-canonical** (design.md Part 7).
...
### W2 — Marketing + auth + entry (M1, A1–A4, N3; A5 retired)

Marketing site. **REPLACED 2026-08-23 (M2, decisions.md D-M2-A..F):** the visitor world is now Abdullah's concept-v2, ported into `src/features/marketing/` — dark, DM Sans + IBM Plex Sans Arabic self-hosted, five routes (`/`, `/pricing`, `/request-demo`, `/terms`, `/privacy`) under one layout route, documented as design.md Part 7. Pricing renders from **marketing's own data module** (`concept/lib/pricing.ts`), not from `src/data/entities/plans.ts` — the two are different documents with different owners (D-M2-B); H1 still reads `usePlans()`. Every "Get started" resolves to the real `/signup` (D-M2-D). The original W2 wording, for the record: light-only, prerendered — hero, features, how-it-works, pricing from the same module H1 reads, FAQ, CTA band. Auth: signup (strength meter + live checklist, duplicate-email → sign-in link), signin (generic error + the **locked-out countdown**), verify (resend cooldown mono timer, expired-link recovery), reset (no-enumeration copy). **A5, the onboarding wizard, is DELETED (ORDER ONB-0827, 2026-08-27, decisions.md D-ONB-C).** Hasan's ruling: signup stays minimal, verifying lands the user IN THE APP, and every piece of setup the wizard used to collect has a durable home instead — brand voice, tones and sources in Settings, the country on I1, the posting rhythm on C1 (which the wizard's deletion leaves as the only schedule surface). The workspace itself is created automatically and idempotently from the org name typed at signup. N3 is no longer a "resume at step N" redirect: it is the workspace-creation RETRY surface for the rare failed or interrupted create, asking for the org name only when none can be recovered. For the record, the wizard was: brand basics → connect (condensed hub; X "coming soon") → calendar (condensed sources) → **Start pipeline** (days pills, posts/day stepper capped at 3, model cards, tones multi-select, live mono summary) → ready (beacon moment).
**Verify:** every A/M state renders per screens4 across the dataset + state switchers · Playwright golden: signup → verify → **the app** (ONB-0827: the wizard step is gone, so the walk is one shorter) · ~~marketing pricing and H1 pricing assert equal from one module~~ **superseded by D-M2-B**: `verify:w02` asserts the SEPARATION instead — nothing under `features/marketing/` imports `@/data` (one exception, the layout's session read) · `posts_per_day=4` blocked with the named message (the cap is declared once in `types.ts`; its control now lives only on C1, so the behavioural check moved to `calendar-connections.spec.ts` with the wizard's deletion) · axe + reduced-motion clean, on all five marketing routes at 1440 and 390.

### W3 — Dashboard + Today + draft detail + media panel + schedule dialog (D1–D5)

D1 stat row (clickable, warning treatments) + quick links + the merged notifications/activity feed. (The setup-incomplete banner pointed at the wizard and went with it, ONB-0827; the readiness checklist card replaces it.) D2 slot-grouped queue: tone badges (custom = preset treatment), collapsible rationale, ClaimChips, event chips, the full status-driven action rows; a failed slot never blocks siblings. D3 two-column detail with the timestamped timeline + judge score. D4 = the Studio composer draft-scoped in a dialog (model picker with plan locks, credit math line, **insufficient-credits inline upsell preserving the prompt**, generating → succeeded/failed states on a timer, credits-released copy on failure). D5 schedule/publish: channel multi-select from posting-allowed connections, per-channel capability warnings, `needs_reauth` row disabled with inline reconnect, explicit per-channel partial results.
...
**Verify:** all five compose scripts render (happy, flagged, failed→recovery, stopped, rate-limited) · knowledge status lifecycle (Uploading→Processing→Ready/Failed+retry) plays through · custom tone appears identically across D2/D3/C1/F1 (one data record, four screens asserted) · honesty note shows for limited platforms · axe.
...
| ---- | ----------------------------------------------------------------------- |
| G1   | W0–W3 demoed (shell, auth, entry, Today with the full state machine)    |
| G2   | W4–W6 demoed                                                            |
