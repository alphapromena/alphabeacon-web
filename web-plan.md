...
- **Datasets replace scenarios.** `src/data/datasets/` composes whole-tenant states — *fresh org*, *active org*, *past due*, *needs re-auth*, *low credits*, *heavy history*. One tap at `/dev/datasets` swaps the dataset the whole app reads. Datasets are the only way a screen reaches a different world.
...
├─ skills/                          # installed AI skills (shadcn/ui) — committed
