...
| `funded/`                               | The three funded proofs on org 1813 (LIVE_MEDIA=1, the founder's word): `captures.json` (with its findings), `findings.md`, the receipts and terminal envelopes under `jobs/`, downscaled `.webp` copies of the images.edit input and both image outputs, the voice timestamps document verbatim. Nothing was deleted on the org. |
