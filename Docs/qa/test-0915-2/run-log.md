...
**Proof (b) — CANCELED.** `3b5d72c`, the run-log line above pushed alone on top of `71d5862` (READY, `dpl_HnQpEKBPpAjpWyn882FPHikoNNQ3`): `dpl_Bzv7QBrAEnDajf2kDZdGcFNXetWA` → **CANCELED** by the ignored build step. Both halves of Phase 1 hold on the branch preview.

**Step 2 — merge.** `main` fast-forwarded `f3d5a83` → `3b5d72c`, `live` fast-forwarded to `main`, both pushed 19:54:10Z; GitHub's `commits/main.atom` and `commits/live.atom` both newest `3b5d72cd9ccb0eac2f75969427a34aa6a6f59ac5`. Stacked branches kept on origin.

**Step 3 — Vercel after the merge (projectId + teamId).** Production for `3b5d72c` (ref `main`, target production): `dpl_6tWGa5F8ypFL6XDnxKisrc8LtuAx` **READY** — built, not canceled, because its base was the last successful production deployment `f3d5a83` and the diff carried code, even though the tip's own commit is docs-only (the trap `ae2d741` would have sprung). The `live` preview for `3b5d72c`: `dpl_4Rvxw4JQXgwPCPTYBwG9gLkBH56q` **READY**, aliased `1.malaky.ai`. Both hosts read at 20:03Z: `2.malaky.ai` and `1.malaky.ai` serve **`index-BVhSvkNX.js` / `index-Crfna5rg.css`** — the same as each other. The last READY branch preview (`71d5862`, `dpl_HnQpEKBPpAjpWyn882FPHikoNNQ3`) serves `index-BWF0pF0M.js` / `index-Crfna5rg.css`: the same CSS, a different JS hash by construction — the branch preview is built without `VITE_API_BASE_URL` (static mode), production and `live` inline the dev base — so "the same as the last READY branch preview" holds for the stylesheet and cannot hold for the script; recorded as such.

**Step 5 — the post-deploy smoke on `2.malaky.ai`, real Chromium** (`post-deploy/smoke.ts` on disk, four runs): run 1 green on everything but its own frame rows; run 2 crashed on a signup submit that answered nothing in 30 s (item 81's shape, on production); run 3 read the frame rows with an add/remove observer and found the rail's inner element replaced 12–23 ms after the tag — at the full-page screenshot the smoke took right after tagging (Playwright resizes the viewport for a full-page capture and shadcn's sidebar re-creates its inner rail element), a harness artefact, while two direct checks on production with no screenshot kept the same rail and main across every hop (login flow, and the exact signup → reload → walk flow); run 4, the screenshot moved before the tag, **green on every row** (`smoke.md`): theme `html.class="dark"`, `--background #080d11`, `--primary #ff4e2d`, DM Sans, no frame on the auth page, entry `index-BVhSvkNX.js`; fresh signup with 000000 → Dashboard, first light played once (2198 ms, item 82) and not on reload; Today, Studio, Settings, Billing inside one frame — same rail, same main, the tagged indicator still the node — with zero console errors during each load; the "Brand voice saved" toast at 13.62:1 from +42 ms; a mid-session 401 (revoke 204) lands on `/login` with the toast, the session purged, the Sign in button up. The four console errors in the total are the revoked token's own 401 resource loads.

**Step 4 — the merge SHA line and state.md**, a docs-only commit on the branch, `main` and `live` fast-forwarded to it: both deployments must be CANCELED and both hosts must still serve `index-BVhSvkNX.js`.

**Step 4 — done.** `675a54a` (the merge line, state.md, docs only) pushed to the branch, `main` and `live` fast-forwarded to it (atom feeds both `675a54aa…`): production `dpl_5sKmz1tFAYVtSLhpdgqZpnWjMdei` **CANCELED**, the `live` preview `dpl_7M8Qty17MaoL8dvT9r1iJUPDS5FP` **CANCELED**, the branch preview `dpl_FAvnvSgbimtDg2UHkVASvUDWee5E` CANCELED — all three by the ignored build step; both hosts still serve `index-BVhSvkNX.js` / `index-Crfna5rg.css`. The serving code commit is `3b5d72c`; the branch tips are `675a54a`. The deployment-id trap is closed on production, on the record.

**Session end 2026-09-15T20:14:19Z.** Every Playwright run at one worker; no `--funded`; orgs 936 / 1867 / 619 untouched; 1813 only through the gate's own files; dev API only; the smoke and its screenshots stay on disk under `post-deploy/`.
