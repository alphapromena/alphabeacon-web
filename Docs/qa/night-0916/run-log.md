...
`main` fast-forwarded `3b5d72c` → **`b1783d6`** (the code tip `60be807` plus the close-out docs commit), `live` fast-forwarded to `main`, both pushed 00:18:13Z; GitHub's `commits/main.atom` and `commits/live.atom` both newest `b1783d6`. Vercel: production `dpl_5Dz1grCLtYQpwupFEEgCcAS1jNi6` READY at `b1783d6` (its base the last successful deployment, `3b5d72c`, so the diff carried code and it built — the deployment-id trap stays closed), the `live` preview `dpl_Bd46EQSVniyYrjssk6dJUtPwE69d` READY at `b1783d6`; the branch preview for the docs commit CANCELED; `2.malaky.ai` and `1.malaky.ai` both serve **`index-Dtgnuihw.js`** (CSS `index-fjbun082.css`).

**Smoke on `2.malaky.ai`** (`test-0916/post-deploy/smoke.md`, real Chromium, 00:21Z, screenshots on disk): the login panel — beacon present, breathing layer `ab-ambient-breathe 20s`, one accent element in the form (Sign in), radius 8 px, no frame; a fresh signup with 000000 → Dashboard, **first light once, 1803 ms paint → gone**, none on reload; Today, Studio, Settings, Billing in one frame with **zero console errors per hop**; the "Brand voice saved" toast at **13.62:1** from +40 ms (the Approve toast needs a draft — draft-dependent moments stay on the static and funded proofs); a deliberate sign-out from Settings → `/`, the M2 hero on screen (`5-signed-out-home.png`), session gone; a mid-session 401 on `/billing` (revoke 204) → login with the toast, and after sign-in **back on `/billing`**. Console errors in total: three, the revoked token's own 401 resource loads.
